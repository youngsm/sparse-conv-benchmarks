...
src/spconv_bench/
  ...
  report.py                # aggregate JSON -> summary.md + CSV + plots (per device)
  compare.py               # cross-cutting plots: vs model size, A100 vs H200
  networks/
...
### How each library scales with model size and GPU

Cross-cutting views (generated by [`spconv_bench.compare`](src/spconv_bench/compare.py); more in [`results/comparison/`](results/comparison)):

![latency vs model size](results/comparison/latency_vs_modelsize.png)

At batch 8, torchsparse++ scales worst with model width (steepest curve) while WarpConvNet/spconv/MinkowskiEngine stay flatter; on H200 the WarpConvNet line is elevated because of its Hopper fallback (see below).

The full picture - latency and memory vs batch size, per model, with A100 (solid) and H200 (dashed) overlaid:

![scaling grid](results/comparison/scaling_grid.png)

Per-network bar/scaling/speedup plots for each GPU are in [`results/plots/`](results/plots) (A100) and [`results/h200/plots/`](results/h200/plots) (H200).
